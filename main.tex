...
\newcommand{\htimage}[2]{\fbox{\includegraphics[width=\linewidth,height=#2,keepaspectratio]{./images/#1.pdf}}}
...
\frametitle{The TensorFlow/XLA Bridge In Action}
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.23\textwidth}
      \small{Mark For Compilation}
    \end{column}
    \begin{column}{0.60\textwidth}
      \small{Encapsulate Subgraphs \& Build XLA Ops}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Auto Clustering \& Deadness}
  \begin{itemize}
  \item Map each node to a symbolic predicate that is true iff the node is execute
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    \begin{column}{0.58\textwidth}
      ...
      \item Resource variables reads and writes are atomic
...
\begin{frame}[fragile]
  ...
  \item We'd love for you to try it out!
    \begin{itemize}
    \item Change the \texttt{TF\_XLA\_FLAGS} environment variable to include \texttt{--tf\_xla\_auto\_jit=2} to enable for all graphs
